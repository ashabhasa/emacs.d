...
(use-package magit                      ; The best Git client out there
  ...
  (add-hook 'projectile-switch-project-hook
            #'arber-magit-set-repo-dirs-from-projectile)
  ...
  (defun arber-magit-kill-buffers ()
  ...
  (bind-key "q" #'arber-magit-kill-buffers magit-status-mode-map))
...
(use-package forge                      ; Access Git forges from Magit
  ...
  :bind ((:map forge-issue-section-map
         ...
         (:map forge-pullreq-section-map
               ("C-c C-v" . forge-browse-topic)))
  :config
  (add-to-list 'forge-alist '("lab.7bridges.eu" "lab.7bridges.eu/api/v4"
                              "lab.7bridges.eu" forge-gitlab-repository)))
